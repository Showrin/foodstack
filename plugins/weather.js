...
(function() {
        ...
        f(document).ready(function(c) {
            if (-1 != document.domain.indexOf("accuweather") || c(".aw-widget-legal").length) {
                ...
                var b = f(e.current.selector).add(e.threeday.selector),
                    a = c();
                b.each(function() {
                    this.awInit || a.length || (a = c(this), this.awType = -1 != a.data("uid").indexOf("awcc") ? "current" : "threeday", this.awInit = !0)
                });
    ...
    function y(c) {
        ...
        c.get(0).awInit = !0;
        f.getJSON(e[c.get(0).awType].url + "?callback=?", b, function(a) {
            c.html(a.html);
            x(f, c)
        });
            ...
            c.autocomplete = function(b, a) {
                ...
                function v(a) {
                    ...
                    for (var c = [], h = 0; h < a.length; h++) {
                        var b, d;
                        a[h] && (b = a[h], d = [b.LocalizedName], "US" == b.Country.ID || "CA" == b.Country.ID || "AU" == b.Country.ID ? d.push(", " + b.AdministrativeArea.ID) : "GB" == b.Country.ID ? void 0 !== b.SupplementalAdminAreas && b.SupplementalAdminAreas.length ? d.push(", " + b.SupplementalAdminAreas[0].LocalizedName) : d.push(", " + b.AdministrativeArea.ID) : b.AdministrativeArea.LocalizedName != b.LocalizedName && d.push(", " + b.AdministrativeArea.LocalizedName), "US" == b.Country.ID ? d.push(" (USA)") : d.push(" (" + b.Country.LocalizedName + ")"), d = d.join(""), b = [], b.push(d), b.push(a[h].Key), c.push(b))
                    }
                    return c
                }
                ...
                function F(c) {
                    ...
                    for (var h = [], d = 0; d < b; d++) h.push('<li class="aw-location"><a data-key="' + c[d][1] + '">' + c[d][0] + "</a></li>");
                ...
                function I(c, b) {
                    ...
                    var d = a.url + b + "?q=" + encodeURI(c) + "&apiKey=" + a.api_key,
                        h;
                    for (h in a.extraParams) d += "&" + h + "=" + encodeURI(a.extraParams[h]);
                    return d
                }

                function H(c) {
                    if (!c) return null;
                    if (q.data[c]) return q.data[c];
                    if (a.matchSubset)
                ...
                function J(b, c) {
                    ...
                    for (var d = c ? c.length : 0, h = null, k = 0; k < d; k++) {
                        ...
                        if (g[0].toLowerCase() == b.toLowerCase()) {
                            ...
                            var e = null;
                            if (1 < g.length)
                                for (var e = [], u = 1; u < g.length; u++) e[e.length] = g[u];
                            h.extra = e
                        }
                    }
                ...
                function u(c, b) {
                    b && c && a.cacheLength && (!q.length || q.length > a.cacheLength ? (t(), q.length++) : q[c] || q.length++, q.data[c] = b)
                }
                ...
                if (null != a.data) {
                    ...
                    for (var B = 0; B < a.data.length; B++) E = "string" == typeof a.data[B] ? [a.data[B]] : a.data[B], 0 < E[0].length && (D = E[0].substring(0, 1).toLowerCase(), A[D] || (A[D] = []), A[D].push(E));
                    for (var z in A) a.cacheLength++,
                        u(z, A[z])
                }
                L.click(function() {
                    f()
                ...
                r.bind("keydown", function(b) {
                    ...
                    switch (b.keyCode) {
                        ...
                        case 13:
                            ...
                            break;
                        default:
                            p = -1, w && clearTimeout(w), w = setTimeout(function() {
                                if (46 == C || 8 < C && 32 > C) n.hide();
                                else {
                ...
                this.findValue = function() {
                    ...
                    d ? J(b, d) : "string" == typeof a.url && 0 < a.url.length ? c.ajax({
                        type: "GET",
    ...
    function x(c, b) {
        ...
        window.accuweather.widgets.ThirtySixHour = function(a) {
            ...
            b.find(".aw-temp-unit").find("li a").each(function(a) {
                c(this).click(function() {
                    c(this).closest("li").hasClass("current") || (c(this).hasClass("aw-unit-f") ? b.data("unit", "f") : b.data("unit", "c"), b.data("useip", "false"), y(b))
                })
            });
            ...
            if (awxWidgetInfo[d].wxInfo)
                for (var k in awxWidgetInfo[d].wxInfo)
                    if ("ix" != k)
                        if ("cu" == k)
                            for (var m in awxWidgetInfo[d].wxInfo[k]) awxWidgetInfo[d].wxInfo[k][m] && e("cu", awxWidgetInfo[d].wxInfo[k]);
                        else if ("fc" == k)
                for (var l = 0; l < awxWidgetInfo[d].wxInfo[k].length; l++)
                    for (m in awxWidgetInfo[d].wxInfo[k][l]) awxWidgetInfo[d].wxInfo[k][l][m] && e("fc" + (l + 1), awxWidgetInfo[d].wxInfo[k][l]);
            else awxWidgetInfo[d].wxInfo[k] && (awxWidgetInfo[d].keyValuePairs[k] = awxWidgetInfo[d].wxInfo[k].toString());
            this.resize();
            window.accuweather.widgets.register(this.el.get(0), this)
        };
        ...
        "current" == b.get(0).awType ? window.accuweatherWidgetCurrentReady && window.accuweatherWidgetCurrentReady(b.get(0), l) : window.accuweatherWidgetThirtySixHourReady && window.accuweatherWidgetThirtySixHourReady(b.get(0), l);
        ...
        try {
            (function(a, b, c, e, d, f, l) {
                ...
                name: "awxoapTracker"
            }), ga_awxoap("awxoapTracker.set", "page", "/oap-weather-widgets/" + f), ga_awxoap("awxoapTracker.send", "pageview", {
